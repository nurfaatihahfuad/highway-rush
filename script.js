...
const game = {
    ...
    // Initialize Game State [cite: 9, 20]
    start() {
        ui.hideAll();
        this.score = 0;
        this.speed = 5;
    ...
    // Main Gameplay Loop [cite: 13, 36]
    loop(timestamp) {
        if (!this.active) return;

        // Enemy Spawning Logic [cite: 35]
        if (timestamp - this.lastSpawn > this.spawnRate) {
            this.spawnEnemy();
            this.lastSpawn = timestamp;
            // Increase intensity over time [cite: 36]
    ...
    spawnEnemy() {
        ...
        el.className = 'car enemy-car'; // Blue/Green style from storyboard [cite: 13]
    ...
    moveEnemies() {
        ...
        this.enemies.forEach((en, i) => {
            ...
            // Collision Detection [cite: 33, 44]
            if (this.checkCollision(playerEl, en)) {
                this.gameOver();
            }

            // Scoring: +1 Point for every car avoided [cite: 31, 51]
            if (top > 600) {
                en.remove();
    ...
    checkCollision(a, b) {
        ...
        // Returns true if cars overlap [cite: 44, 53]
        return !(r1.top > r2.bottom || r1.bottom < r2.top || 
                 r1.right < r2.left || r1.left > r2.right);
    },

    updateScoreUI() {
        document.getElementById('score-display').innerText = `SCORE: ${this.score}`; [cite: 39]
    },
...
/**
 * UI & INTERACTION MANAGEMENT [cite: 4, 17]
 */
const ui = {
...
/**
 * CONTROL LISTENERS [cite: 26]
 */

// Keyboard Controls (A, D, ESC) [cite: 27, 28, 29]
window.addEventListener('keydown', (e) => {
    ...
    if (game.active) {
        ...
        // "Press any key to return to menu" [cite: 37]
